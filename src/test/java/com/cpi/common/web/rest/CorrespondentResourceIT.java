...
public class CorrespondentResourceIT {
    ...
    @Test
    @Transactional
    public void getAllCorrespondentsByContactsIsEqualToSomething() throws Exception {
        // Initialize the database
        CorrespondentContact contacts = CorrespondentContactResourceIT.createEntity(em);
        em.persist(contacts);
        em.flush();
        correspondent.addContacts(contacts);
        correspondentRepository.saveAndFlush(correspondent);
        Long contactsId = contacts.getId();

        // Get all the correspondentList where contacts equals to contactsId
        defaultCorrespondentShouldBeFound("contactsId.equals=" + contactsId);

        // Get all the correspondentList where contacts equals to contactsId + 1
        defaultCorrespondentShouldNotBeFound("contactsId.equals=" + (contactsId + 1));
    }
    ...
    public void getAllCorrespondentsByPortIsEqualToSomething() throws Exception {
        ...
        Port port = correspondent.getPort();
        correspondentRepository.saveAndFlush(correspondent);
