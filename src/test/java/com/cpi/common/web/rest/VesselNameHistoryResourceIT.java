...
/**
 * Integration tests for the {@Link VesselNameHistoryResource} REST controller.
 */
@SpringBootTest(classes = {SecurityBeanOverrideConfiguration.class, CpicommonApp.class})
public class VesselNameHistoryResourceIT {
     ...
     * if they test an entity which requires the current entity.
     */
    public static VesselNameHistory createUpdatedEntity(EntityManager em) {
        VesselNameHistory vesselNameHistory = new VesselNameHistory()
    ...
    public void getAllVesselNameHistoriesByVesselIsEqualToSomething() throws Exception {
        // Initialize the database
        Vessel vessel = VesselResourceIT.createEntity(em);
        em.persist(vessel);
        em.flush();
